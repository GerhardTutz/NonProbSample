...
#### uses two criteria - performs better than one criterion
...
numcont <- 4 ### number continuous var - in the following !!!! means has to be dapted (twice)
numpred <- numcont
...
### true regression
trueint <- 1
truepar <- seq(1,numcont,1)  
truevect <- c(trueint,truepar)
...
pollpar <- truepar+ c(-2,0,1,2 ) ## !!!! lenght numcont
...
for (l in 1: numsim) {
  ...
  ####  end compute criterion beta change
    ...
    for (i in 1:dim) {columns <- numcont+3
                      ...
                      #### two criteria 
                      if ((rs< threshold) & (crit <= critbeta)) {newused$newl[i] <-0 }
                      #### one citerion (less efficient)
                      #if (crit <= critbeta) {newused$newl[i] <-0 }
                      } # end i 
...
###### new scatter against lnear predictor
pred <- samp[,1:numcont]
trueparm <-matrix(truepar,numcont,1)
...
scatterplot(samp$resp~ linpredtrue| lab, data = samp,regLine=FALSE,smooth=FALSE,grid = FALSE, frame = TRUE,
            cex.axis=1.5,cex.lab=1.5,cex.main=1.5, main ="Data versus True Predictor",cex=1.2,
            pch =c(1,2,19), col = c(1,4,2),xlab = "True linear predictor")
...
plot(rates[,1],rates[,2],cex.axis=1.5,cex.lab=1.5,cex.main=1.5, 
     main ="Rates of identified observations across simulations"
     ,cex=1.2,xlab="Correctly identified as member of  target population",
     ylab="Incorrectly identified ")
